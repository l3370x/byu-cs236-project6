...
string Query::postOrder(map<string, Node> & DG, int i) {
	...
	int j = 0;
	if(DG["Q"+UsefulFunctions::convertInt(i)].myChildren.empty() &&
			DG["Q"+UsefulFunctions::convertInt(i)].isQuery) {
...
string Query::backwardEdges(map<string, Node> & DG) {
	...
	set<string> output;
	for (it = DG.begin() ; it != DG.end() ; ++it) {
		cout << "checking at " << (*it).first << endl;
		vector<int> i = (*it).second.hasBackwardEdge(DG);
		string toAdd;
		if(i.size()) {
			toAdd = "    " + (*it).first + ":";
			vector<int>::iterator it2;
			for (it2 = i.begin() ; it2 != i.end() ; ++it2) {
				toAdd += " R" + UsefulFunctions::convertInt((*it2));
			}
			output.insert(toAdd);
		}
	}
